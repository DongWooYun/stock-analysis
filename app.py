...
**배포 개요**
- KOSPI200 + KOSDAQ100 (~300개) 대상 PER 기반 괴리율 분석 대시보드 최초 배포
- DART Open API 연간 EPS 수집 + 분기 TTM YoY 성장률 반영
- 최종 저평가 스크리닝 26개 종목 산출
- Streamlit Cloud 배포

**구현 내용**
- 탭 1: 저평가 스크리닝 (TOP 10, 섹터별 TOP 3, 버블차트, 시그널 필터)
- 탭 2: 괴리율 분석 (기본 vs 조정 비교, 조정폭 TOP 15, 히트맵)
- 탭 3: YoY 성장률 분포 및 산점도
- 탭 4: 종목 상세 (재무 데이터, 게이지 차트)
- 탭 5: 금융 섹터 ROE 기반 별도 분석
- 탭 6: 백테스팅 (시그널별 수익률, ANOVA, SHAP 피처 중요도)
- 공통: 용어 설명, 상단 지표 5개, 시그널 기준 통일
...
**파이프라인 자동화**
- GitHub Actions 스케줄링: 평일 09:00 KST 자동 실행
- DART 2025 사업보고서 자동 감지 및 데이터 갱신
- 현재가 실시간 수집 + 기준일 자동 표시 (상단 info 배너)
- Streamlit 슬립 방지 ping (매일 09:00)

**대시보드 개선**
- 사이드바 추가: 기준일/분석 대상/강력매수 요약 지표, 종목 빠른 검색, 페이지 radio 네비게이션
- 탭 구조 → 사이드바 radio 페이지 전환 방식으로 전환
- 탭 2: 조정 전 분류 기준 expander 위치 이동 (섹터 차트 아래)

**AI 챗봇 추가 (탭 8)**
- Groq API (LLaMA3-70B) 기반 무료 공개 챗봇
- FAQ 즉시 답변 섹션 (API 호출 없음)
- gap_v2.csv 데이터 기반 자유 질문 (예: "삼성전자 왜 강력매수야?")
- 대화 히스토리 유지 + 초기화 버튼
